...
class EEGNetLSTM(nn.Module):
    def __init__(self, eegnet_params, lstm_hidden_dim, num_layers, num_classes=4):
        super(EEGNetLSTM, self).__init__()
        self.eegnet = EEGNet(**eegnet_params)
        self.eeg_feature_dim = self.eegnet.feature_dim()
        self.lstm = nn.LSTM(input_size=self.eeg_feature_dim, hidden_size=lstm_hidden_dim, num_layers=num_layers, batch_first=True)
