...
def train_model_multiclass(model, train_dataloader, val_dataloader, num_epochs, cw_type, device, test_dataloader=None):
    ...
    for epoch in range(num_epochs):
        ...
        val_score_pre = evaluate_f1_selected_classes(model, val_dataloader, device, [PRE_ATTACK])
        ...
        print(f"Epoch [{epoch + 1}/{num_epochs}], Loss: {total_loss / len(train_dataloader):.4f}, Val sc. (f1-preattack): {val_score}, F1-gen: {val_score_gen}")
...
def retrain_model_multiclass(model, train_dataloader, test_dataloader, num_epochs, cw_type, device, exp_name, path_model):
