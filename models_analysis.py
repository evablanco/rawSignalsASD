...
def analyse_PDM_results(path_results, model_code, feats_code, tf, tp, bin_size, split_code):
    # Load the CSV results file ensuring 'Fold' is read as a string
    ...
    def compute_episode_count_and_duration(condition_series, time_series):
        """Compute the number of episodes and their total duration."""
    ...
    # Assign unique colors for each subject
    ...
    # Plotting the relationships in a 3x2 grid
    ...
    for i, (title, column, ax) in enumerate(plot_data):
        ...
        # Remove titles in subfigures
        ax.set_xlabel(column, fontsize=14)
        ax.tick_params(axis='x', labelsize=12)
        ax.tick_params(axis='y', labelsize=12)

        # Y-labels only in the first column
        if i % 2 == 0:
